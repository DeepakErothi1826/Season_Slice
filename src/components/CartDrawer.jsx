...
const CartDrawer = () => {
  ...
  const handleBrowseMenu = useCallback(() => { closeCart(); navigate('/menu'); }, [closeCart, navigate]);
  const handleCheckout = useCallback(() => { closeCart(); navigate('/cart'); }, [closeCart, navigate]);

  if (prefersReduced) {
    if (!isOpen) return null;
    return (
      <>
        <div className="fixed inset-0 bg-coffee/40 backdrop-blur-sm z-50" onClick={handleClose} />
        <aside className="fixed right-0 top-0 h-full w-full sm:w-[420px] bg-cream z-50 shadow-2xl flex flex-col" id="cart-drawer">
          <div className="flex items-center justify-between p-6 border-b border-coffee/10">
            ...
            <button onClick={handleClose} aria-label="Close cart" className="p-2 rounded-full hover:bg-cream-dark transition-colors text-coffee" id="close-cart">
              <X size={20} />
            </button>
          </div>
          <div className="flex-1 overflow-y-auto p-6 space-y-4">
            {items.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-center space-y-4">
                ...
                <button onClick={handleBrowseMenu} className="btn-primary text-sm mt-2">Browse Menu</button>
              </div>
            ) : (
              items.map((item) => (
                <CartItem key={item.id} item={item} onRemove={removeItem} onUpdateQty={updateQuantity} prefersReduced />
              ))
            )}
          </div>
          {items.length > 0 && (
            <div className="border-t border-coffee/10 p-6 space-y-4 bg-cream-light">
              ...
              <button onClick={handleCheckout} className="btn-gold w-full text-center text-base" id="checkout-button">Proceed to Checkout</button>
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-coffee/40 backdrop-blur-sm z-50" onClick={handleClose} />
          <motion.aside
            initial={{ x: '100%' }}
            animate={{ x: 0 }}
            exit={{ x: '100%' }}
...
const CartItem = memo(({ item, onRemove, onUpdateQty, prefersReduced }) => {
  if (prefersReduced) {
    return (
      <div className="flex gap-4 bg-white rounded-xl p-3 shadow-sm">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center justify-between mt-2">
            <div className="flex items-center gap-2 bg-cream-dark rounded-full px-1 py-0.5">
              <button onClick={() => onUpdateQty(item.id, item.quantity - 1)} aria-label={`Decrease quantity of ${item.name}`} className="w-7 h-7 rounded-full flex items-center justify-center hover:bg-cream transition-colors text-coffee">
                <Minus size={14} />
              </button>
              <span className="text-sm font-body font-semibold text-coffee w-5 text-center">{item.quantity}</span>
              <button onClick={() => onUpdateQty(item.id, item.quantity + 1)} aria-label={`Increase quantity of ${item.name}`} className="w-7 h-7 rounded-full flex items-center justify-center hover:bg-cream transition-colors text-coffee">
                <Plus size={14} />
              </button>
            </div>
            <div className="flex items-center gap-3">
              <span className="font-display font-bold text-coffee">{item.price * item.quantity} Rs</span>
              <button onClick={() => onRemove(item.id)} aria-label={`Remove ${item.name} from cart`} className="p-1.5 rounded-full hover:bg-rose/20 text-rose transition-colors">
                <Trash2 size={14} />
              </button>
            </div>
          </div>
