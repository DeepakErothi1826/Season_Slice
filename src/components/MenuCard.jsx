...
const MenuCard = memo(({ item, index = 0 }) => {
  ...
  if (prefersReduced) {
    return (
      <div className="group relative bg-white rounded-xl overflow-hidden hover:shadow-lg transition-all duration-300 border border-gray-100">
        ...
        <Link to={`/product/${item.id}`} className="block" aria-label={`View ${item.name} details`}>
          ...
          <div className="p-3 sm:p-4 select-none">
            ...
            <div className="flex items-center justify-between pt-2 border-t border-gray-100">
              ...
              <span className="text-[10px] sm:text-xs text-gold font-medium uppercase tracking-wider group-hover:text-coffee transition-colors font-bold">View</span>
            </div>
          </div>
        </Link>
      </div>
